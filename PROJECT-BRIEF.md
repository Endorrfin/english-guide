...
| **Definitions (D1)** | The word **STUDY page** (`#/definitions`) over the SAME word corpus (SSOT) — the front door for words (search deep-links here). A–Z + Study/Recall/Describe/Cloze studio + mastery (new/learning/known, SRS-compatible). Owner-approved addition; the Dictionary page stays for now (demote/redirect deferred). |
