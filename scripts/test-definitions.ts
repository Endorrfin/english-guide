...
// ── real corpus invariants (20 D1 + 60 D2 + 60 D3 cards) ──────────────────────────
// CHANGED (D3): custom corpus 20 → 140 (D1 20 + D2 60 + D3 60).
ok(customWords.length === 140, `expected 140 custom cards, got ${customWords.length}`);
...
console.log('✓ test-definitions: all checks passed (A–Z grouping, 4+3 split, cloze, describe-it scaffold on 140 cards).');
