...
> ✅ **DONE — 180 cards this session (2026-07-20), across D2 + D3 + D4.** Shipped **60 × 3 single-word cards** (custom 20 → 200; dictionary → **350 words**) — owner confusables, FEELINGS, work/thinking, character, health, food, weather/nature/animals, connectors; `test-definitions` count 20 → 200; `definition.txt` marked `✅ [D2]`/`[D3]`/`[D4]` + three APPLIED blocks; full `npm run verify` ✓ in a cloud scratch copy after each wave. D2 + D3 merged to `main` via `feat/d2-*` (PR #11) and `feat/d3-*` (PR #12); D4 staged on `main` for `feat/d4-definitions-wave`. Idioms/short phrases stay deferred to a **separate section/tab** (owner's call). Remaining backlog → **D5+**: ~100 single words + the ~178 short-phrase list. The steps below are the standing procedure for the next wave.
