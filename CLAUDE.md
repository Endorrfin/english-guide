...
src/
  ...
  data/      concepts.ts (SSOT aggregator) · modules/mN-*.ts · types.ts
             ...
             idioms.ts (idioms + phrasal verbs) · collocations.ts (V12) · phrases.generated.ts
             (V12: tab-badge counts — VocabTabs must never import a corpus, see §14 V12)
  ...
  lib/       hashRouter.ts · registry.tsx · search.ts (modules + words) · exercise.ts · srs.ts ·
             phrases.ts (shared expression engine, V12) · idioms.ts · collocations.ts (+ the drill) ·
...
(first custom wave). → **D2–D11 (done): +435 definition cards** (dictionary → 605 words; runbook §16). →
...
**V12 (done):** Collocations get their OWN tab, dataset and lazy chunk (`#/collocations`, 162) + the ★ “Which word?” drill; `VocabTabs` counts move to a generated index and a route-isolation gate locks it in. →
...
- **D9 (2026-08-07) — Definitions wave 9: +37 golden single-word cards (custom 385 → 422; dictionary → 572).** Owner's **inline** add-words request (§10 protocol), 39 items given in chat. Dedupe against all word files first — **2 skipped** as already carded under their base lemma (`obstacles` → `obstacle` [D3]; `inferences` → `inference` [D6]; owner chose skip over extending the shipped cards, per the D8 precedent), **4 normalized** to base lemmas per the D6/D7 convention (`hints` → `hint`, `passages` → `passage`, `captures` → `capture`, `endorsements` → `endorsement`), 4 headwords lowercased (`Exactly`, `Shape`, `Ultimately`, `Evidence`). **Owner decision this wave: the -ly adverbs get their OWN cards** (D5 precedent — undoubtedly/unexpectedly/interestingly), so `consistent` · `consistently` · `consistency` ship as a cross-linked trio and `ultimately` sits next to the shipped `ultimate` [D7]. The 37 cards cluster as judgement/evidence (`verdict`, `proof`, `evidence`, `hint`, `distinguish`, `exactly`), growth/character (`aspiration`, `resilience`, `overcome`, `mindset`, `curiosity`, `readiness`, `vision`), business/brand (`revenue`, `equity`, `reputation`, `endorsement`, `presence`, `accomplish`, `transition`), quality/consistency (`consistent`, `consistently`, `consistency`, `robust`, `crucial`, `vital`, `meaningful`, `authentic`, `genuine`, `ultimately`) and study/craft (`scan`, `sketch`, `passage`, `shape`, `capture`, `range`, `matter`). Same golden bar: **US IPA (no slashes, Oxford NAmE style), 7 tagged examples (4 general + 1 each business/office/dev), bilingual `def` + тлумачення, translations, collocations, synonyms/antonyms, in-corpus `seeAlso` (111 resolving links), topics, `source:'custom'`** — appended to `custom.ts`, **no id renumbered**. Authored **sequentially** (owner's choice again — one voice across all 37 cards) with a pre-emptive BrE sweep, then audited by **two parallel `general-purpose` fact-check subagents** against Oxford Learner's / Merriam-Webster (Cambridge was unreachable during the run): **all 37 IPA confirmed correct** — no British forms leaked (`ˈvɜːrdɪkt`, `ˈrevənuː`, `əˈkɑːmplɪʃ`, `ɪnˈdɔːrsmənt`, `ˌkjʊriˈɑːsəti` are all the NAmE variants) — and **~50 corrections applied**, in four kinds. (1) **Levels:** `hint` b1 → b2, `curiosity` b2 → c1, `authentic` b2 → c1 (all three carry an Oxford 5000 badge that disagreed with the draft). (2) **Defs extended to cover a sense one of their own 7 examples used:** `matter` (*what is the matter?*), `hint` (*a hint of lemon*), `scan` (the medical-image noun), `sketch` (a written sketch), `shape` (*in good shape*), `range` (the verb gloss was missing entirely), `robust` (*a robust defense*), `vital` (*vital signs*), `presence` (things/substances, not just people), `endorsement` (the paid-deal sense), `consistency` (parts agreeing — the CAP-style dev example), `distinguish` (*too dark to distinguish* + *distinguish yourself*), `evidence` (the courtroom sense), `meaningful` (*a meaningful look*), `equity` (the draft said *fairness through equal treatment*, which contradicted its own examples 2 and 4). (3) **English defects:** `overcome a scheduling conflict` (wrong collocation — a clash is *resolved*), *reduced vision to fifty meters* (the fixed collocation is *visibility*), *give evidence* (BrE — US courts *present evidence* / *testify*), *the first working day* → *business day*, *training day* → *training session*, *accomplished more in one morning than in the whole week* (the comparison contains its own term), plus the mandative-subjunctive trap on `crucial`/`vital` reworded away rather than switched (*It is crucial to arrive on time*). (4) **Ukrainian:** 15 calques and mistranslations fixed, incl. `Він одна з` → `Він один із` (agreement error), `густина` → `консистенція` on `consistency` (the translations array contradicted the card's own def), `охота` → `бажання` on `readiness` (Russianism), `свідчення` → `докази` where non-legal evidence was meant, `затримала` (detained) → `схопила` on `capture`. One suggestion was **declined and reworded instead**: replacing *overcame the scheduling conflict* with *resolved* would have stripped the headword out of the example and thinned the cloze pool, so the sentence was rebuilt around a correct `overcome` collocation. Only code change beyond data: `scripts/test-definitions.ts` corpus assertion **385 → 422** (+ `gen:index` rewrote `words/index.generated.ts`). Ledger: `✅ [D9]` tags + a **D9 APPLIED block** in `definition_v2.txt`, `DEFINITIONS.md` §6, `CLAUDE.md` §13, `README.md`. **Verification: FULL `npm run verify` ✓ green in a cloud scratch copy** (fresh `npm install`) — typecheck · eslint · check:index · check:data (**572 words = 150 a1 + 422 custom**) · test ×14 (incl. **test-definitions** on 422) · smoke (**305 checks**) · vite build · check:bundle (334.7 kB / 420 kB eager budget); verify was run **twice** — on integration and again after the fact-check corrections. Owner next: `npm run verify` locally → branch `feat/d9-definitions-wave` → commit → PR. **Follow-up still open:** the pre-existing `altitude` card's *two thousand metres* (flagged D8) is still BrE in a US corpus.
...
- **D10 (2026-08-20) — Definitions wave 10: +22 golden single-word cards (custom 422 → 444; dictionary → 594) + an owner-approved BrE sweep of the shipped corpus.** Owner's **inline** add-words request (§10 protocol), 22 items given in chat. Dedupe against ALL word files first — **nothing skipped this wave**: none of the 22 was already carded (the first wave since D7 with a 0% collision rate). **9 normalized** to base lemmas per the D6/D7 convention (`avoiding`→`avoid`, `topics`→`topic`, `subjects`→`subject`, `misunderstandings`→`misunderstanding`, `disagreements`→`disagreement`, `snapshots`→`snapshot`, `expectations`→`expectation`, `qualifications`→`qualification`) and `Occupation` lowercased; the plural collocations the owner actually meets (*manage expectations*, *list your qualifications*, *take snapshots*) are carried in each card's `collocations`, so the lemma rule costs nothing. Owner declined the optional pair-completion cards (`accessibility`, `unconscious`, `conciseness`, `misunderstand`, `disagree`, `unfit`) and the -ly adverbs — this wave is exactly the 22 requested. The cards cluster as clear speech (`accessible`, `jargon`, `concise`, `subject`, `topic`), friction (`controversial`, `misunderstanding`, `disagreement`, `avoid`), judgment/awareness (`wise`, `conscious`, `validate`, `expectation`), work identity (`occupation`, `qualification`, `fit`) and space/record (`terrain`, `elevation`, `snapshot`, `within`, `playful`) — heavily cross-linked into the shipped corpus (94 resolving `seeAlso` links) and into each other. Same golden bar: **US IPA (no slashes, Oxford NAmE style), 7 tagged examples (4 general + 1 each business/office/dev), bilingual `def` + тлумачення, translations, collocations, synonyms/antonyms, in-corpus `seeAlso`, topics, `source:'custom'`** — appended to `custom.ts`, **no id renumbered**. Authored **sequentially** (D8/D9 precedent — one voice), then fact-checked **inline against Oxford Learner's + Merriam-Webster** rather than via subagents (22 cards, one pass): **all 22 IPA confirmed correct** — `əkˈsesəbl`, `ˈdʒɑːrɡən`, `ˌkɑːntrəˈvɜːrʃl`, `ˌmɪsʌndərˈstændɪŋ`, `ˌdɪsəˈɡriːmənt`, `ˌɑːkjuˈpeɪʃn`, `ˈkɑːnʃəs`, `kənˈsaɪs`, `təˈreɪn`, `ˌelɪˈveɪʃn`, `ˈsnæpʃɑːt`, `ˌekspekˈteɪʃn`, `ˌkwɑːlɪfɪˈkeɪʃn` are all NAmE, no British forms leaked. **7 corrections applied:** 4 levels (`expectation` b2 → **c1** on an explicitly quoted *Oxford 5000 / C1* headword badge; `within` b1 → **a2**; `fit` b1 → **a2**; `subject` b1 → **a2** — Oxford badges A1 for `subject`'s core school/topic sense, but the card also teaches *subject to* and the grammar subject, so it sits one notch up, documented rather than silently forced to a1, which would have been the first a1 card in `custom.ts`), 1 EN defect (*a bare code* → *a bare error code*, not idiomatic) and 2 UA fixes (*продажна презентація* reads as **venal**, not *sales* → *презентація для продажів*; *вдалим доповненням* was a loose rendering of *a good fit* → *добре вписався в команду*). **Method note:** Oxford's headword CEFR badge does **not** survive the markdown conversion reliably — several early reads returned a **Topics** badge or a neighbouring entry's badge instead (the first `validate` read returned a B2 that belonged to `perspective`). Only badges quoted verbatim in the *Oxford 5000 / C1* form were treated as authoritative; the rest are authored judgments. Cards with no Oxford badge at all (`validate` c1, `accessible` b2, `concise` c1, `qualification` b2) keep their authored level. **Second workstream — the BrE sweep (owner-approved, closes the follow-up open since D8):** the corpus is US-IPA, so British spellings in it are a defect. A scripted sweep normalized **55** items across shipped `custom.ts` cards: `-our`→`-or` (colour/favour/behaviour/labour/neighbour/honour/flavour/rumour ×22), `-re`→`-er` (`altitude` *two thousand metres*, `bush` *garden centre*, `rope` *by the metre*), `practise`→`practice` on the verb (`emphasize`, `exercise`), `judgement`→`judgment` ×3, `cancelled`→`canceled` ×7, `grey`→`gray` ×7, `towards`→`toward` in the `attitude`/`compassionate` collocations, and the BrE **lexis** `flat`→`apartment` (`purchase`), `lift`→`ride` (`offer`), *on holiday*→*on vacation* ×5 — the last applied **only where the UA gloss said «відпустка»**; the *«свято»/«канікули»* uses (`plummet`, `pattern`, `kennel`, `slack`, `upcoming`, `assignment`, `context`) were kept, since *over the holidays* is standard AmE. The sweep also surfaced a plain article error nobody had caught: `pumpkin`'s *a autumn favourite* → *an autumn favorite*. **No id renamed, no card re-authored, no example semantics changed → SRS progress untouched.** Deliberate BrE cards (`realise`, `recognise` — D6) and variant listings (`amongst` as a synonym of `among`, `towards` on the `toward` card) were left alone on purpose. `a1.ts` was swept with the same patterns and came back **clean** (its only two hits — *after the holidays* and `vacation`'s *holiday* synonym — are both correct). Code changes beyond data: `scripts/test-definitions.ts` corpus assertion **422 → 444**, and `npm run gen:index` rewrote `words/index.generated.ts` (which `check:index` correctly failed on first, as designed). **Verification: FULL `npm run verify` ✓ green in a cloud scratch copy** (fresh `npm install` — device `node_modules` are macOS-native) — typecheck · eslint · check:index · check:data (**594 words = 150 a1 + 444 custom**) · test ×13 (incl. **test-definitions** on 444) · smoke · vite build · check:bundle (337.6 kB / 420 kB eager budget); verify was run **twice** — once on integration, once after the fact-check corrections and the BrE sweep. Owner next: `npm run verify` locally → branch `feat/d10-definitions-wave` → commit → PR. **No follow-up left open from D8/D9** — the `altitude` BrE item is closed.

- **V12 (2026-08-21) — Collocations become their own Words tab (`#/collocations`), +57 entries, + the ★ "Which word?" drill.** The 117 collocations that shipped inside `data/idioms.ts` as `kind:'collocation'` (V10) moved **verbatim** to a new **`src/data/collocations.ts`** (`COLLOCATIONS`, ids untouched) with a new route, page and **lazy chunk**; `data/idioms.ts` is now idioms + phrasal verbs only. **Ids and the `idiom:<id>` mastery prefix are unchanged, and the SRS deck stays `idioms`** (label only becomes “Idioms & collocations”) — re-prefixing would have reset the owner's progress on 117 cards (PROJECT-BRIEF §10). **⚠ The finding that made the split real:** `VocabTabs` imported `WORDS` + `IDIOMS` + `IRREGULAR` for its count badges, and every Words route renders it — so opening `#/dictionary` already downloaded the whole idioms corpus, and adding `COLLOCATIONS` would have loaded the new chunk on all five tabs, making the split **purely cosmetic**. `check:bundle` could not see it (it walks the EAGER graph; every Words page is behind `React.lazy`). Fixed by generating **`src/data/phrases.generated.ts` (`PHRASE_COUNTS`)** in `gen-data-index.ts`, and locked by a new **route-isolation gate** in `check:bundle` that walks each route chunk's static graph and fails if a page reaches a corpus it does not use — **tested against a deliberate regression** (re-adding the import turns 4 pages red; §12 rule). Result: `WordsLayout` is 1.4 kB and pulls no corpus; idioms chunk **268 → 180 kB**, new collocations chunk **140 kB**, eager payload 337.6 → **339.8 kB / 420 kB**. **Code split:** `lib/phrases.ts` (shared engine — kind grouping, themes, guess blank, shuffle, match rounds, `phraseOfDay`), `lib/idioms.ts` (idiom categories only), **`lib/collocations.ts`** (8 group ids + the drill); `components/words/{PhraseCard,PhraseModes,phraseMeta}` extracted so both pages share one card/Guess/Match implementation. **Data (+57):** **45 new collocations** from the owner's list — 4 in the new **`soft-skills`** category (+4 retro-tagged from `business`: give a presentation · give feedback · raise concerns · address an issue) and 12 in the new **`everyday`** category, the rest into the existing buckets — plus **11 phrasal verbs** (log in/out · wake up · get up · turn on/off · look after · look for · give up · go away · take off) and **1 idiom** (`in deep water`, filed as an idiom because a bare “deep water” teaches nothing). 27 of the owner's 85 items were already in the corpus; 4 near-duplicates became **`synonyms` cross-links** instead of cards (`meet deadlines` → `meet a deadline`, `have a break` → `take a break`, `do the washing up` → `do the dishes`, `highly recommend` → `highly recommended`). `collGroupAdvAdj` relabelled **“Adverb + Adjective / Verb”** (the id stays) because the bucket also holds adverb+VERB pairs. **★ Which word?** — `buildPickRound()` blanks the head word and draws distractors from the corpus; the first build offered `ground · peak · state` for `___ deadline` (same *semantic* category, wrong word class), so `entryHeadClass()` now recovers the slot class per entry — grammatical category where it says so, else the house “To …” meaning shape (97.6% agreement measured against the 127 entries whose category already answers it) — and `headClassIndex()` maps head → *set* of classes, because `close` heads both `close a deal` and `close friend`. Distractors are same-class, same-category-first, and never a phrase that is itself correct. **CSS:** the tab bar is sized from the **Ukrainian** labels (the content column is capped at 863px at every viewport, so no media query can help; five roomy tabs needed 984px) — one row in both languages, EN 814px / UA 845px. **Gates:** `check:data` + the new `scripts/test-phrases.ts` validate the **union** of both files (id + phrase uniqueness across files, and each kind in the right file); `test-collocations.ts` is new (8 categories populated, 157 class-pure drill rounds); `test-idioms.ts` slimmed; `test-srs.ts` covers the two-file deck. **Verification: FULL `npm run verify` ✓ green in a cloud scratch copy** — typecheck · eslint clean · check:index · check:data (**414 expressions = 183 idiom · 69 phrasal · 162 collocation**) · test ×14 · smoke (**312 checks**) · build · check:bundle. Real-browser check in headless Chromium (five tabs on one row, category bar, drill correct + reveal, no console errors). Owner next: `npm run verify` locally → branch `feat/v12-collocations` → commit → PR. Deferred: per-category counts on the chip bar; a POS tag on collocation entries would replace the “To …” heuristic; the `group` field still mixes grammatical and semantic axes (a `domain` split is the eventual fix, see `lib/collocations.ts`).

- **LE1 (2026-08-21) — Reading wave from a text drop: +25 texts, 298 → 323; the "Learn English" reel series.** First §15 wave sourced from a **plain text file** (`_examples/Reading/reading new.txt`, 27 blocks) rather than screenshots, so steps 2–3 of the runbook (ASCII-renaming, image staging, subagent fan-out) were skipped entirely and the whole wave ran **inline**: parse → dedupe → author → integrate → verify. **The dedupe finding that mattered:** this is a single-author series built on one boilerplate frame (*"Hello everyone. Today I want to talk about the importance of X in our life…"* · *"Students, workers, and leaders all need X to succeed."* · *"In short, X is the key to a happy and successful life."* · a closing reflective question). Raw token-Jaccard over EN bodies therefore scored **new×new at .35–.53 on texts that share nothing but the frame** — applying §15's ≥ .45 rule mechanically would have discarded ~10 legitimate texts (Discipline×Hard work .50, Resilience×Determination .53). Re-scoring with the frame's ~50 stopwords removed collapsed **every** new×new pair below **.30** and left exactly **one** true duplicate against the shipped corpus: `Discipline` vs the shipped `discipline-is-the-key-to-success` (**.477** content-only) — skipped. **26 → 25 shipped.** `Template` (a 2-word placeholder block) dropped. **Split:** discipline 9 · values 7 · mindset 6 · family 1 · study 1 · friendship 1 — no new rubric, no `categories.ts` or `index.ts` change. **Levels:** 21 b1 · 4 a2 (`the-most-valuable-thing-we-have`, `the-hardest-person-to-control`, `my-english-learning-journey`, `what-makes-a-true-friend`); **minutes** 20×1 · 5×2, computed from the *final* body word count (≤130 → 1) — the first pass computed it from the raw block, which included the closing question, and over-rated five texts. **Attribution:** all 25 → `Learn English` (facebook.com), each carrying its own **per-text reel URL**; a new author vs §15's list. **Titles/ids are all renamed off the source headings** (`Courage` → `Courage Means Moving Even When Afraid`, `Father` → `A Father's Silent Support`, …) because 8 of the 26 source headings collide verbatim with shipped titles.

  **Two owner decisions, taken up front.** (1) The source is an auto-transcript with **22 word-level speech-to-text defects**; owner chose *fix silently* over §15's "verbatim EN", so the shipped English corrects them without an in-file changelog: `successful like`→`life` ×2, `in comes from`→`it comes`, `a tool than can`→`that can`, `a save place`→`safe`, `Weather in studies`→`Whether`, `per assistant effort`→`persistent effort`, `It was afraid`→`I was afraid`, `I our daily life`→`In our daily life`, `the important of failure`→`importance`, `only some became special`→`become`, `achieve great result`→`results`, `Grateful person`→`A grateful person`, `In the short,`→`In short,`, `the importance of goal`→`goals`, `continue your work`→`continuing`, a duplicated `Hello everyone.` stutter, and one fragment rejoined (`…move forward. Even when we face problems.`). Source contractions (`don't`) and the speaker's own sentence breaks were **left alone** — the rule was "fix mis-hearings, not style". A word-diff of every record against its source block is the audit trail. (2) The **closing reflective question** each text ends on is carried into the record's `open` question rather than left in the body — so the body ends on substance and the question does the work it was written for.

  **QA — three adversarial passes, and the second one is why this took a second integration.** Pass 1 (2 reviewers × 12–13 records, EN fidelity · UA fidelity · house style · mcq correctness · metadata): **EN fidelity, paragraph structure, punctuation house style and every `correct` index came back clean**, but both reviewers independently flagged the Ukrainian as carrying a systematic **calque layer** — `дати велику різницю` ("make a big difference") ×5, `будує` + abstract nouns ×6, and a `Чого одного…/На що одне…/Чим одним…` frame calqued from EN "one thing" across **18** open questions — plus real mistranslations: `має велику вартість` (*costs a lot*, inverting "has a big value"), `маємо це на увазі` for *mean it*, `витрачаємо на … заощадження` (*we spend it on savings*), `зривають проєкти` (*wreck projects*, for neutral "fail in projects"), `відкатами` (*kickbacks*, for "setbacks"), `владу сказати «ні»` (*authority over others*, for "the power to say no to yourself"), `Витривалість` (*physical stamina*) as the headword for **perseverance**, and `спікер` (*a parliamentary speaker*) for a first-person narrator. Pass 1 also caught a design defect no data check can see: **4 records printed their own MCQ answer in their title** (`Sacrifice Is the Bridge` → "What image does the text use for sacrifice?") and 3 more questions were answerable from world knowledge alone. Pass 2 (a native-level Ukrainian editor over the corrected set, replacements required to be drop-in) returned **14 MAJOR + 44 MINOR** — including four cases where the *edit pass itself* had introduced term drift between a body and its own comprehension question (`незламність` in the body vs `витривалість` in the question), two MCQ items left grammatically broken (`Ми не завжди можемо **його** контролювати` where the antecedent is plural `зміни`; a `Чого… стосується` stem whose options all began `Про…`), and ти/ви drift inside 6 records. **All 58 applied.** Net: ~90 Ukrainian edits, 7 MCQ items rewritten, 1 title changed (`Time, Once Gone, Never Returns` → `The Most Valuable Thing We Have`, because the title restated the answer to its own first question). **Mechanical gates written for this wave** and kept in the wave scratch: an EN↔UA char-ratio sweep (all 76 paragraph pairs **0.87–1.16**, zero outliers), a sentence-count parity check (zero mismatches), a house-style linter that exempts `explain.uk` (by convention an English quotation inside « »), and a **title↔answer leak check** (Jaccard of the correct option against the record title) — the last one is new and worth folding into `check:data` if a future wave repeats the pattern.

  **Verification: FULL `npm run verify` ✓ green in a cloud scratch clone** (fresh `npm install` — device `node_modules` are macOS-native), run **twice**: once on first integration, once after the Ukrainian editing pass. typecheck · eslint (0 warnings) · check:index · check:data (**323 reading texts in 22 categories**, all bilingual, registry + links resolve) · test ×14 · smoke **312** · vite build · check:bundle (**eager 339.8 kB / 420 kB — unchanged; the reading corpus stays lazy**). `npm run gen:index` rewrote `reading/index.generated.ts` 298 → 323 (`check:index` correctly failed on it first, as designed). **Watch item — the §15 scale gate is close:** the lazy `reading` chunk is now **1,486 kB** (1,419 kB at S2). Still fully lazy so the eager budget is untouched, but §15 puts the **per-category lazy split** at ~400 texts and this wave spent a quarter of the remaining headroom. Files written back to the device; the agent did **not** commit or push (mount blocks `unlink` — owner commits locally, per the S2/D10 pattern). The source file `_examples/Reading/reading new.txt` was **left in place, unmodified** — nothing was archived or moved. Owner next: `npm run verify` locally → branch `feat/le1-reading-wave` → commit → PR.

- **D11 (2026-09-24) — a mixed inline drop routed by TYPE: +11 definition cards · +41 collocations · +8 phrasal verbs · +2 idioms (62 entries).** Owner asked for a Definitions wave, but the list mixed three kinds of item. Routed per the Words-hub SSOT (owner confirmed: *split by type, unique only*): **single words → `words/custom.ts`** (custom 444 → **455**, dictionary → **605**: align · struggle · humility · lack · notably · ridiculous + agenda · input · outweigh · roadmap · consensus — the last five came from the owner's *collocations* block but are single words); **collocations → `data/collocations.ts`** (162 → **203**) placed by `COLLOCATION_GROUP_RULE` into the existing 8 groups (soft-skills 11 · business 13 · workplace 3 · verb-noun 8 · adjective-noun 5 · adverb-adjective 1 — no new category, so no code change); **phrasal verbs + idioms → `data/idioms.ts`** (252 → **262**: elaborate on · get back to · run something by someone · chime in · come up · follow up · bring up · take on; idioms `set your sights on` [success-failure] · `move up the career ladder` [work-business]). **Skipped as duplicates of shipped entries** (10): touch base · back up · take responsibility · take minutes · raise a concern (= `raise concerns`) · tight deadline(s) · wrap up/wrap-up · carry out · point out · turn down; `engage listeners` folded into `engage the audience` as a synonym. **Owner glosses corrected in the data, not copied:** *case study* «вивчення випадку» → «кейс», *visual aid* «візуальна допомога» → «наочний матеріал», *project milestone* → «ключовий етап / віха», typos (eleborate, proglem-solving, transiton, передвати). **Correctness trap made explicit:** *table a proposal* means *put forward* in BrE but *postpone* in AmE — the corpus is AmE, so the card teaches both with a Common-mistake note and safe alternatives. Collocation verbs keep the house **“To …” meaning shape** so `entryHeadClass()` classes them correctly for the ★ Which-word drill (test-collocations: 198 rounds, all class-pure, no self-defeating distractor). Levels: Oxford badges used only where quoted for the headword (align b2, notably b2, ridiculous c1); the rest authored (humility/outweigh/consensus c1). **Verification: FULL `npm run verify` ✓ green** in a scratch copy on the owner's machine VM (fresh `npm ci`) — typecheck · eslint · check:index · check:data (**605 words · 262 idioms/phrasals · 203 collocations**) · test ×14 · smoke (312) · build · check:bundle (341.3 / 420 kB). Data-only wave: no component/CSS change; `test-definitions` 444 → 455; `gen:index` rewrote `words/index.generated.ts` + `phrases.generated.ts`. Ledger: `_examples/definition_v2.txt` D11 block (✅ [D11] + routing per line).
...
>
> ✅ **Wave LE1 DONE (2026-08-21) — a TEXT drop: `reading new.txt` → +25 texts, 298 → 323.**
> When the drop is a text file, **skip steps 2–3 entirely** (no ASCII renaming, no image staging,
> no subagent fan-out) and run parse → dedupe → author → integrate → verify **inline**; §14's LE1
> entry is the worked example. Two corrections to the steps below came out of it, both in step 4:
>   1. **Dedupe on content, not on frame.** A single-author series that reuses one boilerplate
>      opening/closing scores **.35–.53 raw Jaccard between texts that share nothing but the
>      template**, so the ≥ .45 rule below silently kills real texts. Strip the frame's stopwords
>      first, then apply the threshold — content-only scoring collapsed all 325 new×new pairs
>      below .30 and isolated the one genuine duplicate at .477.
>   2. **Check the title against its own answer key.** Renaming a text to a distinct title (step 4)
>      can make the title restate the correct MCQ option, which no data gate catches. Score the
>      correct option against the title and rewrite one of them above ~.34.
