// CHANGED (V12): the SHARED pure engine for multi-word expressions — everything the Idioms tab
// (#/idioms) and the Collocations tab (#/collocations) both need, extracted from lib/idioms.ts when
// collocations got their own dataset and page. No React, no side effects, no DATA imports (only the
// `IdiomEntry` type), so both lazy chunks can pull it for free and scripts/test-phrases.ts can
// import it under tsconfig.node.json.
//
// What stays OUT of here: anything specific to one corpus. Idiom categories live in lib/idioms.ts,
// collocation groups + the "Which word?" drill live in lib/collocations.ts.
import type { IdiomEntry, IdiomKind } from '../data/types';

export const IDIOM_KINDS: readonly IdiomKind[] = ['idiom', 'phrasal', 'collocation'];

/** Sorted, de-duplicated theme tags across a list — drives the theme filter on both pages. */
...
/**
 * A stable "phrase of the day": deterministic pick by a day index (same all day, rotates daily).
 * Each page passes its OWN corpus, so the Idioms tab and the Collocations tab feature different
 * entries on the same day.
 */
export function phraseOfDay(list: readonly IdiomEntry[], dayIndex: number): IdiomEntry | null {
