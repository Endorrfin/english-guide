// CHANGED (S1): block renderer for the 7 content kinds (ported from ../database guide).
// Language-guide tone mapping (contract tones stay verbatim, display labels adapt):
//   tip → Tip · warn → Watch out · senior → Advanced (C1-level nuance) · security → Register
//   (formality/politeness alerts — the language-guide analogue of a security warning).
...
const TONE_LABEL: Record<string, Localized> = {
  ...
  senior: { en: 'Advanced', uk: 'Поглиблено' },
  security: { en: 'Register', uk: 'Реєстр мовлення' },
};
...
function CompareBlock({ a, b, rows }: { a: Localized; b: Localized; rows: [Localized, Localized, Localized][] }) {
  const { t } = useLang();
  return (
    <div className="dtable-wrap">
